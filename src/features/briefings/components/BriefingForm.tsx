...
const BriefingForm: React.FC<BriefingFormProps> = ({ formId }) => {
    ...
    return (
        <div className="h-screen w-full flex flex-col font-sans relative overflow-hidden" style={bgStyle}>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 w-full max-w-4xl mx-auto h-full relative z-10">
                {currentStep === -1 ? (
                    /* Welcome */
                    <div className="w-full">
                        <h1 className={`text-4xl md:text-5xl font-bold mb-6 ${textPrimary}`} style={{ color: isLight ? undefined : themeColor }}>{briefing.title}</h1>
                        {briefing.description && <p className={`text-xl md:text-2xl mb-12 leading-relaxed max-w-2xl ${textSecondary}`}>{briefing.description}</p>}
                        <button onClick={handleNext}
                            className="px-8 py-4 rounded-xl text-white font-bold text-lg md:text-xl transition-all hover:scale-105 active:scale-95 shadow-lg flex items-center gap-2"
                        ...
                        <div className={`mt-8 flex items-center gap-2 text-sm font-medium ${navHint}`}><Clipboard className="w-4 h-4" /> {questions.length} perguntas</div>
                    </div>
                ) : q && (
                    /* Question */
                    <div key={currentStep} className="w-full">
                        <div className="flex items-center gap-4 mb-4">
                            <span className="text-xl md:text-2xl font-bold opacity-50" style={{ color: themeColor }}>{currentStep + 1} &rarr;</span>
                            <h2 className={`text-2xl md:text-4xl font-bold leading-tight ${textPrimary}`}>
                                {q.questionText}{q.isRequired && <span className="text-red-500 ml-2">*</span>}
                            </h2>
                        </div>
                        {q.description && <p className={`text-lg md:text-xl mb-8 ml-10 md:ml-12 leading-relaxed ${textSecondary}`}>{q.description}</p>}

                        <div className="ml-10 md:ml-12 mt-8 w-full max-w-2xl">
                            {(q.type === 'short_text') && (
                                <input type="text" placeholder="Digite sua resposta aqui..." value={answers[q.id] || ''}
                                    onChange={e => setAnswers({ ...answers, [q.id]: e.target.value })}
                                    className={`w-full text-2xl md:text-3xl bg-transparent border-b-2 ${inputBorder} p-2 pb-4 outline-none transition-colors`} autoFocus />
                            )}
                            {q.type === 'email' && (
                                ...
                                return (
                                    <div className="flex items-end gap-3">
                                        ...
                                        <input type="tel" inputMode="numeric" placeholder="(00) 00000-0000" value={raw}
                                            ...
                                            className={`flex-1 text-2xl md:text-3xl bg-transparent border-b-2 ${inputBorder} p-2 pb-4 outline-none transition-colors`} autoFocus />
                                    </div>
                                );
                            })()}
                            {q.type === 'date' && (() => {
                                    ...
                                    <div className="flex items-end gap-3">
                                        <div className="flex flex-col">
                                            ...
                                            <input type="text" inputMode="numeric" maxLength={2} placeholder="DD" value={day}
                                                ...
                                                className={`w-20 text-2xl md:text-3xl bg-transparent border-b-2 ${inputBorder} p-2 pb-4 outline-none text-center transition-colors`} autoFocus />
                                        </div>
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <input type="text" inputMode="numeric" maxLength={2} placeholder="MM" value={month}
                                                ...
                                                className={`w-20 text-2xl md:text-3xl bg-transparent border-b-2 ${inputBorder} p-2 pb-4 outline-none text-center transition-colors`} />
                                        </div>
                                        <span className={`text-3xl font-light pb-4 ${textSecondary}`}>/</span>
                                        <div className="flex flex-col">
                                            ...
                                            <input type="text" inputMode="numeric" maxLength={4} placeholder="AAAA" value={year}
                                                ...
                                                className={`w-28 text-2xl md:text-3xl bg-transparent border-b-2 ${inputBorder} p-2 pb-4 outline-none text-center transition-colors`} />
                                        </div>
                                    </div>
                                );
                            })()}
                            {q.type === 'long_text' && (
                                <textarea placeholder="Digite sua resposta detalhada aqui..." value={answers[q.id] || ''}
                                    onChange={e => setAnswers({ ...answers, [q.id]: e.target.value })}
                                    className={`w-full text-xl md:text-2xl bg-transparent border-b-2 ${inputBorder} p-2 outline-none transition-colors min-h-[150px] resize-none`} autoFocus />
                            )}
                                    ...
                                    {q.options?.map((opt, idx) => {
                                        ...
                                        return (
                                            <button key={idx}
                                                ...
                                                className={`w-full text-left px-6 py-4 rounded-xl border-2 transition-all text-lg md:text-xl font-medium flex items-center justify-between
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${sel ? 'bg-white shadow-sm' : letterNormal}`}
                                                        style={sel ? { color: themeColor } : {}}>{String.fromCharCode(65 + idx)}</div>
                                                    {opt}
                                                </div>
                                                {sel && <Check className="w-6 h-6" style={{ color: themeColor }} />}
                                            </button>
                                        );
                                    })}
                                </div>
                                    ...
                                    {q.options?.map((opt, idx) => {
                                        ...
                                        return (
                                            <button key={idx}
                                                ...
                                                style={sel ? { backgroundColor: themeColor + '15', color: themeColor, borderColor: themeColor } : {}}>
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-8 h-8 rounded-md flex items-center justify-center font-bold text-sm ${sel ? 'bg-white shadow-sm' : letterNormal}`}
                        ...
                        <div className="ml-10 md:ml-12 mt-8 flex items-center gap-4">
                            <button onClick={handleNext} disabled={isSubmitting}
                                className="px-6 py-3 rounded-xl text-white font-bold text-lg transition-all hover:opacity-90 active:scale-95 shadow-md flex items-center gap-2"
                                style={{ backgroundColor: themeColor }}>
